            ...
            {view === "register" && (
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8, marginBottom: 14 }}>
                {[["buyer", "Buy minis", "Browse & commission"], ["creator", "Sell my work", "Open a studio"]].map(([role, t, s]) => (
                  <button key={role} onClick={() => setForm({ ...form, role })} style={{ textAlign: "left", padding: 14, borderRadius: 12, cursor: "pointer", background: form.role === role ? "rgba(217,140,63,.1)" : PANEL2, border: `1px solid ${form.role === role ? ACCENT : LINE}` }}>
